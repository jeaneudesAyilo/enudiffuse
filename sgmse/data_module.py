...
from sgmse.util.utils_video import get_mouthroi_audio_pair_load_raw_video
...
def getTAUnoise(subset):

    """get list of noise file paths to perform training or validation """

    all_noise_path = "/group_storage/source_separation/TAU_Urban_Acoustic_Scenes_16k_16bit"

    df_noise_path = {"train": "/group_storage/source_separation/TAU_Urban_Acoustic_Scenes_16k_16bit/meta/TAU-urban-acoustic-scenes-2019-development/evaluation_setup/fold1_train.csv",
                "valid": "/group_storage/source_separation/TAU_Urban_Acoustic_Scenes_16k_16bit/meta/TAU-urban-acoustic-scenes-2019-development/evaluation_setup/fold1_test_add_scene_label.csv",
                # "test" : "/group_storage/source_separation/TAU_Urban_Acoustic_Scenes_16k_16bit/meta/TAU-urban-acoustic-scenes-2019-development/evaluation_setup/fold1_evaluate.csv"
                }
    
    df_noise = pd.read_csv(df_noise_path[subset], sep="\t")

    noise_labels = np.array(["airport", "bus", "metro", "metro_station", "park",  \
                            "public_square", "shopping_mall",  "street_pedestrian",  "street_traffic",  "tram"])

    np.random.seed(42)
    train_noise_labels = list(np.random.choice(noise_labels, size=6, replace=False))

    set_of_noise = df_noise[df_noise["scene_label"].isin(train_noise_labels)]["filename"].tolist() # train : 5552 ; val : 2487

    # if subset=="valid":
    #     set_of_noise = list(np.random.choice(set_of_noise, size=360, replace=False)) #sel 360 val file to make 1h of audio

    ## path in set_of_noise are relative paths, get the absolute paths
    
    noise_absolute_paths = []
    for relative_noisefile_path in set_of_noise:

        for i in range(1,22):
            absolute_path = os.path.join(all_noise_path, f"audio_{i}", "TAU-urban-acoustic-scenes-2019-development", relative_noisefile_path)
            if os.path.isfile(absolute_path):
                noise_absolute_paths.append(absolute_path)
                break
            else:
                absolute_path=None
        ...
        if absolute_path is None:  
            ...
            raise FileNotFoundError(
                errno.ENOENT, os.strerror(errno.ENOENT), relative_noisefile_path)

    
    return noise_absolute_paths  
...
class Specs(Dataset):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        assert not joint_noise_clean_speech_training, "This dataset class is used only when we don't train jointly on clean speech and noise"

        if format == "tcd-timit" :

            if not noise_modelling:
                data_dir = "/group_storage/audio_visual/TCD-TIMIT/"
                t = "_data_NTCD"
                self.clean_files = getTIMITclean(subset, data_dir, t)

                # data_dir = "/group_storage/audio_visual/TCD-TIMIT_original/volunteers"                
                # self.clean_files = sorted(find_files(os.path.join(data_dir, subset), ext='wav'))  

              
            else :
                #in fact here we will be training on demand noise and filter out TMETRO noise, as TCD-DEMAND test set contain TMETRO noise
                print("learning on noise")
                data_dir = "/group_calculus/users/jayilo"
                clean_files = sorted(glob(join(data_dir, subset) + '/clean/*.wav'))

                self.clean_files = sorted([f for f in clean_files if "TMETRO" not in f])

                noisy_files = sorted(glob(join(data_dir, subset) + '/noisy/*.wav'))

                self.noisy_files =sorted([f for f in noisy_files if "TMETRO" not in f])                    
            ...
            if noise_modelling:
                ...
                dic_noisy = {
                        ...
                        "valid": join(noisy_dir,"val"),
                        "test": join(noisy_dir,"test"),
                }                             
                self.noisy_files = sorted(find_files(dic_noisy[subset], ext='wav'))            

            assert audio_only==True , print(f"If format == 'wsj0' video is not available")


        elif format == "vb":
            self.clean_files = find_files(join(data_dir, subset,"clean"), ext="wav")
            
            if noise_modelling:
                self.noisy_files = find_files(join(data_dir, subset,"noisy"), ext="wav")            


             
        elif format == "tau":            
            assert noise_modelling
            assert audio_only
            self.noise_files = getTAUnoise(subset)
        ...
        if vfeat_processing_order == "cut_extract":
            ...
            if format =="tcd-timit":
                ...
                if video_feature_type in ["avhubert", "resnet", "raw_image",]:
                    self.video_size=88
                    self.video_path  = "/group_storage/audio_visual/CROPPED_MOUTH_ldmark_48_68_size_88_88/TCD-TIMIT/{subset}/{speaker_id}/straightcam/{filename}_mouthcrop.mp4"                
                                
                if video_feature_type == "flow_avse":
                    self.video_size=112
                    self.video_path  = "/group_storage/audio_visual/CROPPED_MOUTH_ldmark_28_68_size_112_112/TCD-TIMIT/{subset}/{speaker_id}/straightcam/{filename}_mouthcrop.mp4" 
        ...
        if self.noise_modelling:
            assert self.vfeat_processing_order == "default"
            assert self.audio_only == True


    def __getitem__(self, i):            
       
        if self.audio_only and self.vfeat_processing_order == "default": 
            
            if self.format in ["tcd-timit" ,"wsj0", "vb"]: #we eventuelly need the clean speech file to get the noise (substract the clean from noisy)
                x, _ = load(self.clean_files[i])  


                if self.noise_modelling:
                    if self.format=="tcd-timit":
                        assert os.path.basename(self.clean_files[i])==os.path.basename(self.noisy_files[i])

                    elif self.format=="vb":
                        assert self.clean_files[i].split("/")[-1] == self.noisy_files[i].split("/")[-1]
            ...
            elif self.format in ["tau"]:  
                x, _ = load(self.noise_files[i])
            ...
            raise NotImplementedError(f"Not implemented __gretitem__ when audio_only is {self.audio_only} and vfeat_processing_order {self.vfeat_processing_order}")



    def __len__(self):
        
        if (self.format in ["tau"]):
            if self.dummy:
                # for debugging shrink the data set size
                return int(len(self.noise_files) / 200)
            else:
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--transform_type",
            type=str,
            choices=("exponent", "log", "none", "normalise","normalise_real_im_max"),
            default="exponent",
    ...
    def __init__(
        ...
        impose_batch_1 = False,
        noise_modelling = False,
        joint_noise_clean_speech_training=False,
        **kwargs,
    ):
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            self.train_set = Specs(
                ...
                impose_batch_1 = self.impose_batch_1,
                noise_modelling = self.noise_modelling,
                joint_noise_clean_speech_training = self.joint_noise_clean_speech_training,
                **specs_kwargs, ## specs_kwargs already contained video_feature_type and joint_noise_clean_speech_training
            )
    ...
    def spec_fwd(self, spec):
        ...
        elif self.transform_type == "normalise_real_im_max":        
            spec_decomplex = torch.cat(
                (torch.abs(spec.real), torch.abs(spec.imag)), dim=0
            ) 
            spec = spec/torch.max(spec_decomplex)        
