...
class SeparateParaDiffUSEEN:
    ...
    def predictor_corrector(self, St, v, t, laststep, dt, noise=False):
    ...
    def posterior_sampler(self, startstep=0, S0=None, N0=None):  
        ...
        if N0 is None:
            Nt = (
                torch.randn_like(self.X) * self.sde_noise._std(timesteps[startstep]*torch.ones(1, device=self.device))
                + N_mean
            )
        else:
            Nt = (
    ...
    def run(
        ...
        if not self.listen_noise:
            ...
            if self.verbose and clean_file != None:
                ...
                ## This is only for verification purpose, the estimated clen speech to be considered is St (in stft)  ans st (in waveform)
