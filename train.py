...
from sgmse.data_module_joint import SpecsDataModuleJoint
...
if __name__ == "__main__":
    ...
    for parser_ in (base_parser, parser):
        ...
        parser_.add_argument(
            "--noise_modelling",
            action="store_true",
            help="Specify if a score model is used to model noise. By default a noise model is not used, this correspond to not specifying --noise_modelling",
        )        
    ...
    if temp_args.initial_joint_noise_clean_speech_training:
        data_module_cls = SpecsDataModuleJoint 
    ...
    arg_groups["ScoreModel"].joint_noise_clean_speech_training = temp_args.initial_joint_noise_clean_speech_training
    ...
    model = ScoreModel(
        ...
        **{
            ...
            **{
                ...
                "noise_modelling": args.noise_modelling,
                # "initial_joint_noise_clean_speech_training":args.joint_noise_clean_speech_training,
    ...
    if args.no_wandb:
        logger = TensorBoardLogger(save_dir=f"logs", name="tensorboard") 
    else:
    ...
    callback_last_ckpt = ModelCheckpoint(dirpath=f"logs/{logger.version}", save_last=True, filename="{epoch}-last")
    callback_valid_loss = ModelCheckpoint(dirpath=f"logs/{logger.version}", save_top_k=1, monitor="valid_loss", mode="min", filename="{epoch}-{valid_loss:.2f}")
    callbacks = [callback_last_ckpt, callback_valid_loss]
